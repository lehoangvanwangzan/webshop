...
async function seedBrands() {
  console.log('Seeding brands → shop_brand...');
...
async function seedCategories() {
  console.log('Seeding parent categories → shop_categories...');

  let totalChildren = 0;

  for (const parent of parentCategories) {
    const savedParent = await prisma.category.upsert({
    ...
    console.log('Seeding child categories → shop_child_categories...');
  ...
  console.log(`✓ ${parentCategories.length} parent categories + ${totalChildren} child categories`);
}

// ─── MAIN ─────────────────────────────────────────────────────────────────────

async function seedUsers() {
  console.log('Seeding users → shop_users...');
